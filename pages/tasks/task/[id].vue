<template>
  ...
  <div v-if="isEditingActive && task.chain_conditions[0]" class="task-sidebar-card on-chain">
          <TaskOnChain
            @update-selected-blockchain="(value) => (selectedBlockchain = value)"
            @update-selected-condition="(value) => (selectedCondition = value)"
            @update-currency-address="(value) => (currencyAddress = value)"
            @update-contract-address="(value) => (contractAddress = value)"
            @update-receiver-address="(value) => (receiverAddress = value)"
            @update-amount-condition="(value) => (amountCondition = value)"
            @update-sender-address="(value) => (senderAddress = value)"
            @update-token-id="(value) => (tokenId = value)"
            @update-button-link="(value)=> actionButtonLink = value"
            @update-button-title="(value)=> actionButtonTitle = value"
            @update-token-name="(value) => conditionTokenName = value"
            @update-is-native-token="(value) => (isNativeToken = value)"
            @update-nft-name="(value) => nftName = value"
            @update-nft-preview="(value) => nftImage = value"
...
const selectedCondition = ref(conditions.length > 0 ? conditions[0] : null);
const selectedBlockchain = ref(blockchains.length > 0 ? blockchains[0] : null);
...
const tokenId = ref();
const conditionTokenName = ref('');
const actionButtonLink = ref('')
const actionButtonTitle = ref('')
const isNativeToken = ref(false);
const nftName = ref('');
const nftImage = ref('');
const currencyAddress = ref('');
const amountCondition = ref(0);
...
const sendOnChain = async () => {
  ...
  if (!isErrors) {
    formData.append('name', selectedCondition?.value?.value);

    const getValuesOnChain = {
      [ON_CHAIN_CONDITION_PARTS.CONTRACT_ADDRESS]: contractAddress?.value,
      [ON_CHAIN_CONDITION_PARTS.BLOCKCHAIN]: selectedBlockchain?.value?.value,
      [ON_CHAIN_CONDITION_PARTS.SENDER_ADDRESS]: senderAddress.value,
      [ON_CHAIN_CONDITION_PARTS.RECEIVER_ADDRESS]: receiverAddress.value,
      [ON_CHAIN_CONDITION_PARTS.AMOUNT]: amountCondition.value,
      [ON_CHAIN_CONDITION_PARTS.CURRENCY_ADDRESS]: currencyAddress.value,
      [ON_CHAIN_CONDITION_PARTS.TOKEN_ID]: tokenId.value,
      [ON_CHAIN_CONDITION_PARTS.TOKEN_NAME]: conditionTokenName.value,
      [ON_CHAIN_CONDITION_PARTS.IS_NATIVE_TOKEN]: isNativeToken.value,
      [ON_CHAIN_CONDITION_PARTS.LINK_TO_COMPLETION]:{
        actionButtonLink: actionButtonLink.value,
        actionButtonTitle: actionButtonTitle.value,
      },
      [ON_CHAIN_CONDITION_PARTS.NFT_PREVIEW]: {
          imgUrl: nftImage.value,
          nftName: nftName.value,
        }
    };

    let conditionIndex = 0;
    ON_CHAIN_CONDITION_PARTS_MAP[selectedCondition?.value?.value].forEach((key) => {
...
const validationCheck = (key, isTriggered) => {
  ...
  const tokenIdValue = tokenId.value;
  const selectedConditionValue = selectedCondition.value;

  switch (key) {
    case 'contractAddressOnChain':
    if (!isNativeToken.value && fieldInTheFormScope(ON_CHAIN_CONDITION_PARTS.CONTRACT_ADDRESS, selectedConditionValue)) {
    ...
    case 'amountCondition':
      const amountConditionValue = amountCondition.value;
    ...
    case 'senderAddress':
      ...
      break;
    case 'tokenId':
      if (fieldInTheFormScope(ON_CHAIN_CONDITION_PARTS.TOKEN_ID, selectedCondition.value)) {
...
watch(contractAddress, () => {
  validationCheck('contractAddressOnChain');
});
watch(senderAddress, () => {
  validationCheck('senderAddress');
});

watch(receiverAddress, () => {
  validationCheck('receiverAddress');
});

watch(amountCondition, () => {
  validationCheck('amountCondition');
});

watch(currencyAddress, () => {
  validationCheck('currencyAddress');
});

watch(tokenId, () => {
  validationCheck('tokenId');
});
...
watch(task, (value) => {
  ...
  if (findConditionOnChain?.name) {
    ...
    if (findConditionState) {
      selectedCondition.value = findConditionState;
    }
  }
...
const clearOnChain = () => {
  if (task) {
    ...
    if (value?.chain_conditions[0]?.name) {
      const findConditionState = conditions.find((el) => {
        return el?.value === value.chain_conditions[0].name;
      });
    ...
    if (value?.chain_conditions[0]?.chain_condition_fields) {
      value?.chain_conditions[0]?.chain_condition_fields?.map((el) => {
        if (el?.name === ON_CHAIN_CONDITION_PARTS.BLOCKCHAIN) {
          const findBlockchainsState = blockchains.find((blockchain) => {
            return blockchain?.value === el.value;
          });
