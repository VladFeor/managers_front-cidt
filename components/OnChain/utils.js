...
export const getConditionDescription = (condition) => {
  ...
  switch (condition.name) {
    case ON_CHAIN_CONDITIONS.USER_HOLDS_TOKEN: {
      ...
      return `You must hold at least <span style="font-weight: 500">${amount} ${token ? token : 'token'} 
    ...
    case ON_CHAIN_CONDITIONS.USER_HOLDS_POAP: {
      ...
      return `You must hold <span style="font-weight: 500">${token_id ? `#${token_id}` : 'any'} ${nftName || 'POAP'}</span> on <span style="font-weight: 500">${blockchain}</span>.`;
    }
