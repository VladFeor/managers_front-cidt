  ...
  <div class="create-task_wrapper">
    ...
    <div v-if="props.typeTask.id === 'on-chain'" class="section_wrapper">
      ...
      <TaskOnChain
        ...
        @update-nft-name="(value) => conditionNftName = value"
        @update-nft-preview="(value) => conditionNftPreview = value"
    ...
    <div v-if="props.typeTask.id === 'on-chain'" class="section_wrapper" style="gap: 4px">
...
const conditionNftName = ref('')
const conditionNftPreview = ref('');
...
watch(actionButtonLink, () => {
  validationCheck('actionButtonLink');
});

watch(actionButtonTitle, () => {
  validationCheck('actionButtonTitle');
});
...
const validationCheck = (key, isTriggered) => {
  ...
  switch (key) {
    case 'projects':
      ...
      break;
    case 'blockchains':
      if (!isOnChain && !blockchainId.value && isCoinRewardActive.value) {
        if (isTriggered) {
          field.isTriggered = isTriggered;
        }
        field.isError = true;
    ...
    case 'tokenId':
      if (
        isOnChain &&
        fieldInTheFormScope(ON_CHAIN_CONDITION_PARTS.TOKEN_ID, selectedCondition.value)
      ) {
    ...
    case 'actionButtonLink':{
      const res = validateByKey('actionButtonLink', actionButtonLink.value)
    ...
    case 'actionButtonTitle':{
      const res = validateByKey('actionButtonTitle', actionButtonTitle.value)
      if(typeof res === 'boolean') {
        field.isError = false
        field.text = ''
      } else {
...
const send = async () => {
    ...
    if (props.typeTask.id === ON_CHAIN_TASK) {
      const getValuesOnChain = {
        ...
        [ON_CHAIN_CONDITION_PARTS.NFT_PREVIEW]: {
          imgUrl: conditionNftPreview.value,
          nftName: conditionNftName.value,
        }
      };
  ...
  ON_CHAIN_CONDITION_PARTS_MAP[selectedCondition?.value?.value].forEach((key) => {
    ...
    if (value) {
        ...
        formData.append(`chain_condition[fields][${conditionIndex}][value]`, value);
          ...
          Object.keys(value).forEach((k)=> {
            if(value[k]) {
              ...
              formData.append(`chain_condition[fields][${conditionIndex}][value]`, value[k].toString()); 
              conditionIndex++;
            }
          })
          }
      ...
      formData.append('chain_condition[name]', selectedCondition?.value?.value);
